...
class TestSQLite:
    ...
    def test_ddl_applied(self, sqlite):
        ...
        expected = ["anomalies", "feedback", "feature_schemas", "models", "raw_vectors", "settings"]
    ...
    def test_execute(self, sqlite):
        """Test execute method."""
        sqlite.execute("INSERT INTO settings (key, value) VALUES (?, ?)", ("test_key", "test_value"))
        row = sqlite.fetchone("SELECT value FROM settings WHERE key = ?", ("test_key",))
        assert row["value"] == "test_value"
    
    def test_fetchone(self, sqlite):
        """Test fetchone method."""
        sqlite.execute("INSERT INTO settings (key, value) VALUES (?, ?)", ("key1", "value1"))
    ...
    def test_tx_commit(self, sqlite):
        ...
        with sqlite.tx() as conn:
            conn.execute("INSERT INTO settings (key, value) VALUES (?, ?)", ("tx_key", "tx_value"))
    ...
    def test_tx_rollback(self, sqlite):
        ...
        try:
            with sqlite.tx() as conn:
                conn.execute("INSERT INTO settings (key, value) VALUES (?, ?)", ("roll_key", "roll_value"))
    ...
    def test_row_to_dict(self, sqlite):
        ...
        sqlite.execute("INSERT INTO settings (key, value) VALUES (?, ?)", ("dict_key", "dict_value"))
...
class TestRepository:
    ...
    def test_upsert_anomaly(self, repo):
        ...
        # Insert anomaly
        repo.upsert_anomaly("A1", "2025-11-04T10:00:00Z", "sensor-1", "schema1", now_iso(), now_iso())
    ...
    def test_upsert_anomaly_update(self, repo):
        ...
        # Update
        new_time = now_iso()
        repo.upsert_anomaly("A1", "2025-11-04T11:00:00Z", "sensor-2", "schema1", now_iso(), new_time)
    ...
    def test_list_anomalies(self, repo):
        ...
        # Insert multiple
        for i in range(5):
            repo.upsert_anomaly(f"A{i}", f"2025-11-04T10:0{i}:00Z", "sensor-1", "schema1", now_iso(), now_iso())
    ...
    def test_list_anomalies_by_source(self, repo):
        ...
        repo.upsert_anomaly("A1", "2025-11-04T10:00:00Z", "sensor-1", "schema1", now_iso(), now_iso())
        repo.upsert_anomaly("A2", "2025-11-04T10:01:00Z", "sensor-2", "schema1", now_iso(), now_iso())
        repo.upsert_anomaly("A3", "2025-11-04T10:02:00Z", "sensor-1", "schema1", now_iso(), now_iso())
    ...
    def test_latest_feedback(self, repo):
        ...
        repo.insert_feedback("F1", "A1", "user1", "TP", 0.9, None, "2025-11-04T10:00:00Z")
        repo.insert_feedback("F2", "A1", "user1", "FP", 0.8, None, "2025-11-04T10:01:00Z")
    ...
    def test_list_feedback(self, repo):
        ...
        repo.insert_schema("schema1", "128", 1, 128, "float32", now_iso())
        repo.upsert_anomaly("A1", "2025-11-04T10:00:00Z", "sensor-1", "schema1", now_iso(), now_iso())
        repo.upsert_anomaly("A2", "2025-11-04T10:01:00Z", "sensor-1", "schema1", now_iso(), now_iso())
    ...
    def test_iter_vectors(self, repo):
        ...
        for i in range(3):
            aid = f"A{i}"
            repo.upsert_anomaly(aid, f"2025-11-04T10:0{i}:00Z", "sensor-1", "schema1", now_iso(), now_iso())
    ...
    def test_count_vectors(self, repo):
        ...
        for i in range(3):
            ...
            repo.upsert_anomaly(aid, now_iso(), "sensor-1", schema_id, now_iso(), now_iso())
    ...
    def test_model_operations(self, repo):
        ...
        # Insert model
        repo.insert_model("AE-2025.11.04-1", "dense", "schema1", "artifacts/AE-2025.11.04-1", now_iso())
    ...
    def test_foreign_key_cascade(self, repo):
        ...
        # Setup
        repo.insert_schema("schema1", "128", 1, 128, "float32", now_iso())
        repo.upsert_anomaly("A1", now_iso(), "sensor-1", "schema1", now_iso(), now_iso())
