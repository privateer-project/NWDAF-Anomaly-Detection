...
def test_artifacts():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Verify format
        assert "dense" in version.lower() or version.startswith("dense_")
        print(f"  - Version format: {version}")

        # Test same-day versioning
        version2, _ = artifacts.create_version("dense", (128,))
        print(f"✓ Second version: {version2}")
        assert version != version2
        print("  - Different timestamps verified")
        ...
        print(
            f"  - Model file: model.pt ({(version_dir / 'model.pt').stat().st_size} bytes)"
        )
        ...
        threshold = {"value": 0.5, "percentile": 99.5, "method": "mse"}
        ...
        for (name1, p1), (name2, p2) in zip(
            model.named_parameters(), loaded["model"].named_parameters()
        ):
            assert name1 == name2
        ...
        try:
            ...
            print("✓ Correctly raised error for missing model")

        try:
            artifacts.load_config("nonexistent-version")
            print("❌ Should have raised ArtifactNotFound")
        except ArtifactNotFound:
            print("✓ Correctly raised error for missing config")
        ...
        print(
            f"  - Max difference: {(output_original - output_loaded).abs().max():.2e}"
        )
