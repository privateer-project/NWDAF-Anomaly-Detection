...
class AnomalyDetectorWithUI:
    ...
    def process_message(self, input_data):
        ...
        try:
            ...
            input_tensor = torch.tensor(input_features.values, dtype=torch.float32).to(
                self.device
            )
            ...
            true_label = input_data["metadata"].get(
                "attack", 0
            ...
            if is_anomaly_detected:
                ...
                should_alert, count = self.should_send_alert(
                    input_data["device_id"], input_data["timestamp"]
                )
                ...
                if should_alert:
                    alert = {
                        ...
                        "reconstruction_error": detection_results[
                            "reconstruction_error"
                        ],
                    ...
                    logging.info("ALERT sent for network sequence")
            ...
            for feature_name, feature_sequence in input_data["feature_values"].items():
                if feature_name in self.input_features:
                    ui_feature_values[feature_name] = feature_sequence[
                        -1
                    ]  # Latest value
    ...
    def should_send_alert(self, device_id, timestamp):
        ...
        self.device_alerts[device_id] = [
            t for t in self.device_alerts[device_id] if t > cutoff_time
        ]
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H1(
                            "🛡️ PRIVATEER Network Anomaly Detection",
                            className="text-center mb-4",
                        ),
                        html.P(
                            "Real-time Privacy-Preserving Anomaly Detection for 6G Networks",
                                    ...
                                    [
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                dcc.Slider(
                                                    id="threshold-slider",
                                                    min=detector.initial_threshold
                                                    * 0.5,
                                                    max=detector.initial_threshold
                                                    * 1.5,
                                                    step=0.001,
                                                    value=detector.threshold,
                                                    marks={
                                                        detector.initial_threshold
                                                        * 0.5: f"{detector.initial_threshold * 0.5:.3f}",
                                                        detector.initial_threshold: f"{detector.initial_threshold:.3f}",
                                                        detector.initial_threshold
                                                        * 1.5: f"{detector.initial_threshold * 1.5:.3f}",
                                                    },
                                                    tooltip={
                                                        "placement": "bottom",
                                                        "always_visible": True,
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                dbc.Badge(
                                                    "Network Aggregation Active",
                                                    color="success",
                                                    className="ms-2",
                                                ),
                                                html.Small(
                                                    " - Cross-device aggregated analytics",
                                                    className="text-muted ms-2",
                                                ),
                                            ]
                                        ),
                                        html.Div(
                                            id="status-indicator", className="mt-3"
                                        ),
                                    ]
                                )
                            ]
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H4(
                                            "🌐 Network Status", className="card-title"
                                        ),
                                        html.Div(
                                            id="network-status",
                                            style={
...
@app.callback(
    ...
    [Input("threshold-slider", "value"), Input("detector-state", "data")],
)
def update_status_indicator(threshold, state):
    ...
    return html.Div(
        [
            ...
            dbc.Badge(
                f"Model: {detector.model_name}", color="primary", className="ms-2"
            ),
        ]
    )
...
def update_graphs(n, state):
    if not state.get("running", False):
        ...
        return (
            empty_fig,
            empty_fig,
            html.P("Start detection to see statistics"),
            html.P("Network monitoring inactive"),
        )
    ...
    for data_point in latest_data:
        ...
        detector.realtime_data["reconstruction_error"].append(
            data_point["reconstruction_error"]
        )
    ...
    if len(detector.realtime_data["timestamp"]) > detector.max_points:
        for key in detector.realtime_data:
            if key != "feature_values":
                detector.realtime_data[key] = detector.realtime_data[key][
                    -detector.max_points :
                ]
            else:
                for feature in detector.realtime_data[key]:
                    detector.realtime_data[key][feature] = detector.realtime_data[key][
                        feature
                    ][-detector.max_points :]
    return (
                        ...
                        [
                            dbc.CardBody(
                                [
                                    html.H5("🚨 Detected"),
                                    html.H3(
                                        f"{detector.stats['anomalies_detected']}",
                        ...
                        [
                            dbc.CardBody(
                                [
                                    ...
                                    html.Small(
                                        f"TP: {detector.stats['true_positives']}"
                                    ),
                                ]
                            )
                        ]
                        ...
                        [
                            dbc.CardBody(
                                [
                                    ...
                                    html.Small(
                                        f"FP: {detector.stats['false_positives']}"
                                    ),
                                ]
                            )
                        ]
...
def create_network_status():
    ...
    anomaly_rate = (
        (anomaly_sequences / total_sequences) * 100 if total_sequences > 0 else 0
    )

    color = (
        "danger" if anomaly_rate > 50 else "warning" if anomaly_rate > 20 else "success"
    )
            ...
            dbc.ListGroupItem(
                [
                    html.Div(
                        [
                            html.Span(
                                "Network Aggregated Analytics", className="fw-bold"
                            ),
                            dbc.Badge(
                                f"{anomaly_rate:.1f}%",
                                color=color,
                    ...
                    html.Small(
                        f"Total Sequences: {total_sequences}, Anomalous: {anomaly_sequences}"
                    ),
                ]
            ),
            dbc.ListGroupItem(
...
if __name__ == "__main__":
    ...
    logging.info(
        f"📏 Threshold Range: {detector.initial_threshold * 0.5:.3f} - {detector.initial_threshold * 1.5:.3f}"
    )
