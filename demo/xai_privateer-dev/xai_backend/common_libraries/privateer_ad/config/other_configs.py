...
class PathsConf:
    ...
    analysis: Path = Path(
        os.environ.get("ANALYSIS_DIR", root.joinpath("analysis_results"))
    )
    raw_dataset: Path = Path(
        os.environ.get(
            "RAW_DATASET", raw.joinpath("amari_ue_data_merged_with_attack_number.csv")
        )
    )
    experiments_dir: Path = Path(
        os.environ.get("EXPERIMENTS_DIR", root.joinpath("experiments"))
    )
