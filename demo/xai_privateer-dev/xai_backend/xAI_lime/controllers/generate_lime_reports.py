...
def generate_json_report_lime(
    explanation, path_json, instance_values=None, feature_names=None, model_output=None
):
    """
    ...
    report = {
        "sample": {
            name: float(val) for name, val in zip(feature_names, instance_values)
        }
        if instance_values and feature_names
        else {},
        "lime_weights": {
...
def main_generate_all_lime_reports(
    data_lime, instance_index=0, base_dir="reports_lime"
):
    """
    ...
    generate_txt_report_lime(
        explanation, os.path.join(folders["txt"], base_name + ".txt")
    )
    generate_csv_report_lime(
        explanation, os.path.join(folders["csv"], base_name + ".csv")
    )
    generate_xlsx_report_lime(
        explanation, os.path.join(folders["xlsx"], base_name + ".xlsx")
    )
    generate_json_report_lime(
        explanation,
        os.path.join(folders["json"], base_name + ".json"),
        instance_values,
        feature_names,
    )
    generate_html_report_lime(
        explanation, os.path.join(folders["html"], base_name + ".html")
    )
    generate_pdf_report_lime(
        explanation, os.path.join(folders["pdf"], base_name + ".pdf")
    )
    generate_md_report_lime(explanation, os.path.join(folders["md"], base_name + ".md"))

    print(
        f"[INFO] All LIME reports for instance {instance_index} generated successfully."
    )
