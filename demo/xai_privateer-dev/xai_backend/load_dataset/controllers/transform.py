...
class DataProcessor:
    ...
    def split_data(self, df, train_size=0.8):
        ...
        for device, device_info in self.metadata.devices.items():
            ...
            device_val_df, device_test_df = train_test_split(
                df_tmp, test_size=0.5, stratify=df_tmp["attack_number"], random_state=42
            )
    ...
    def get_partition(self, df: DataFrame, partition_id=0) -> DataFrame:
        ...
        return partitioner.load_partition(partition_id).to_pandas(batched=False)[
            df.columns
        ]
    ...
    def _read_csv(self, path):
        ...
        try:
            return pd.read_csv(
                path, dtype=self.metadata.get_features_dtypes(), parse_dates=["_time"]
            )
        except FileNotFoundError:
            raise FileNotFoundError(f"File {path} not found.")

    def get_dataloader(
        self, path, batch_size=1024, seq_len=6, partition_id=0, only_benign=False
    ) -> DataLoader:
        """
    ...
    def get_dataloader2(
        self, data, batch_size=1024, seq_len=6, partition_id=0, only_benign=False
    ) -> DataLoader:
        """
