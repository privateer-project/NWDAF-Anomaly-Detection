...
def generate_json_report_shap(data, path_json, instance_index=0):
    ...
    report = {
        ...
        "shap_values": {
            name: safe_float(val) for name, val in zip(data["feature_names"], shap_vals)
        },
        "contribution": [
            {"feature": name, "value": safe_float(val)}
            for name, val in zip(data["feature_names"], shap_vals)
        ],
        "model_output": data.get(
            "model_output"
        ),  # Adiciona isto no main_calculation_shap
    }
...
def main_generate_all_shap_reports(
    data_shap, instance_index=0, base_dir="reports_shap"
):
    ...
    generate_txt_report_shap(
        data_shap, os.path.join(folders["txt"], base_name + ".txt"), instance_index
    )
    generate_csv_report_shap(
        data_shap, os.path.join(folders["csv"], base_name + ".csv"), instance_index
    )
    generate_xlsx_report_shap(
        data_shap, os.path.join(folders["xlsx"], base_name + ".xlsx"), instance_index
    )
    generate_json_report_shap(
        data_shap, os.path.join(folders["json"], base_name + ".json"), instance_index
    )
    generate_html_report_shap(
        data_shap, os.path.join(folders["html"], base_name + ".html"), instance_index
    )
    generate_pdf_report_shap(
        data_shap, os.path.join(folders["pdf"], base_name + ".pdf"), instance_index
    )
    generate_md_report_shap(
        data_shap, os.path.join(folders["md"], base_name + ".md"), instance_index
    )

    print(
        f"[INFO] All SHAP reports for instance {instance_index} were generated successfully."
    )
