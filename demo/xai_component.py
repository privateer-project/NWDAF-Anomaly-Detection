...
class XAIComponent:
    ...
    def _load_model(self) -> nn.Module:
        ...
        model_path = (
            model_dir / f"{self.config.get('MODEL_CURRENT_VERSION', 'latest')}.pt"
        )
        ...
        if attention_ae_path.exists():
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    "attention_ae", attention_ae_path
                )
                attention_ae_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(attention_ae_module)

                # Get model classes
                AttentionAutoencoder = getattr(
                    attention_ae_module, "AttentionAutoencoder", None
                )
                AttentionAutoencoderConfig = getattr(
                    attention_ae_module, "AttentionAutoencoderConfig", None
                )
        ...
        class LSTMAutoencoder(nn.Module):
            ...
            def forward(self, x):
                ...
                decoder_input = torch.zeros(
                    (x.size(0), x.size(1), hidden.size(2)), device=x.device
                )
        ...
        if model_path.exists():
            try:
                model.load_state_dict(torch.load(model_path))
                logger.info(
                    f"Loaded model weights into fallback model from {model_path}"
                )
            except Exception as e:
    ...
    def _calculate_feature_importance(self, inputs: torch.Tensor) -> Dict[str, float]:
        ...
        for i, feature_name in enumerate(self.feature_list):
            ...
            perturbed_input[:, :, i] = (
                perturbed_input[:, :, i] * 1.1
            )  # 10% perturbation

            # Calculate new error
            with torch.no_grad():
                perturbed_output = self.model(perturbed_input)
                perturbed_error = torch.mean(
                    torch.pow(perturbed_input - perturbed_output, 2)
                ).item()
        ...
        if total > 0:
            feature_importances = {
                k: abs(v) / total for k, v in feature_importances.items()
            }
    ...
    def _calculate_attention_weights(self, inputs: torch.Tensor) -> List[float]:
        ...
        if hasattr(self.model, "transformer_encoder") and hasattr(
                ...
                with torch.no_grad():
                    ...
                    if hasattr(last_layer, "self_attn"):
                        ...
                        if hasattr(last_layer.self_attn, "attn_weights"):
                            ...
                            avg_weights = (
                                attn_weights.mean(dim=(0, 1)).cpu().numpy().tolist()
                            )
                            return avg_weights
            except Exception as e:
    ...
    def _generate_explanation(self, data: Dict) -> Dict:
        ...
        try:
            ...
            for data_point in sequence_data:
                features = [
                    float(data_point.get(feat, 0.0)) for feat in self.feature_list
                ]
            ...
            if data.get("is_anomaly", False):
                explanation_text = (
                    f"Anomaly detected with score {data.get('anomaly_score', 0):.4f}. "
                )
                ...
                explanation_text += (
                    f"The anomaly was most evident at time index {max_time_idx}."
                )
            else:
                explanation_text = "No anomaly detected. "
                explanation_text += "The data shows normal patterns in all features."
    ...
    def run(self):
        ...
        try:
            for message in self.consumer:
                ...
                logger.info(
                    f"Received explanation request: {request.get('request_id', 'unknown')}"
                )
                ...
                logger.info(
                    f"Published explanation for request: {request.get('request_id', 'unknown')}"
                )
...
def main():
    ...
    try:
        ...
        config = {
            ...
            "KAFKA_XAI_REQUEST_TOPIC": os.environ.get(
                "KAFKA_XAI_REQUEST_TOPIC", "xai-request"
            ),
            "KAFKA_XAI_RESPONSE_TOPIC": os.environ.get(
