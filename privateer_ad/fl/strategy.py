...
class CustomStrategy(FedAvg):
    ...
    def aggregate_fit(
        ...
        if (metrics_aggregated and
            self.training_config.target_metric in metrics_aggregated and
            self._is_better_metric(metrics_aggregated[self.training_config.target_metric])):

            self.best_target_metric = metrics_aggregated[self.training_config.target_metric]

            set_weights(self.model, parameters_to_ndarrays(parameters_aggregated))

            logging.info(f"New best model at round {server_round} with {self.training_config.target_metric}: {self.best_target_metric:.5f}")
