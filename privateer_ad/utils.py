...
def load_model_weights(model_path: str, paths_config) -> dict:
        ...
        if path.exists():
            ...
            try:
                ...
                if isinstance(state_dict, dict):
                    ...
                    raise ValueError(
                        f"Unexpected state dict format: {type(state_dict)}"
                    )
    ...
    raise FileNotFoundError(
        f"Could not find model file at any of: {[str(p) for p in possible_paths]}"
    )
...
def log_model(
    ...
    try:
        ...
        try:
            champions = client.search_model_versions(
                filter_string=f"name = '{model_name}' and tag.champion = 'true'"
            )
        except Exception as e:
            logging.warning(
                f"No previous champion found or error accessing registry: {e}"
            )
    ...
    if is_champion:
        logging.info("🏆 New champion model! Logging with champion tag.")

        # Remove champion tag from previous versions
        for champion in champions:
            try:
                client.delete_model_version_tag(
                    model_name, champion.version, "champion"
                )
                logging.info(f"Removed champion tag from version {champion.version}")
            except Exception as e:
                logging.warning(
                    f"Failed to remove champion tag from version {champion.version}: {e}"
                )

        try:
            client.set_model_version_tag(
                model_name, current_version, "champion", "true"
            )
        ...
        logging.info(
            f"Model performance ({current_target_metric}) did not exceed "
            f"previous best ({best_target_metric}). No champion tag added."
        )

    client.set_model_version_tag(
        model_name, current_version, target_metric, str(current_target_metric)
    )
...
def get_signature(model, sample):
    ...
    signature = mlflow.models.infer_signature(
        model_input=sample.detach().numpy(), model_output=_output
    )
    return signature
    ...
    try:
        ...
        for key in threshold_keys:
            if key in run.data.metrics:
                ...
                logging.info(
                    f"📏 Found threshold: {threshold:.6f} (from metric: {key})"
                )
                break
        ...
        loss_fn_name = run.data.params.get(
