...
class ModelTrainer:
    ...
    def __init__(
        ...
        self.loss_fn = getattr(torch.nn, self.training_config.loss_fn_name)(
            reduction="none"
        )
    ...
    def _validation_loop(self, val_dl) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            for inputs in progress_bar:
                ...
                y_score.extend(
                    batch_y_score_per_sample.cpu().tolist()
                )  # reconstruction error per sample
                progress_bar.set_postfix(
                    {"val_loss": torch.mean(batch_y_score_per_sample).item()}
                )
        ...
        n_benign_y_scores = self.rng.permutation(x=benign_y_scores, axis=0)[
            :n_samples_per_class
        ]
        n_malicious_y_scores = self.rng.permutation(x=malicious_y_scores, axis=0)[
            :n_samples_per_class
        ]
        ...
        balanced_y_true = np.concatenate(
            [
                np.zeros(n_samples_per_class, dtype=np.int32),
        ...
        fpr, tpr, thresholds = roc_curve(
            y_true=balanced_y_true, y_score=balanced_y_scores
        )
        ...
        balanced_metrics = self.calculate_metrics(
            rec_errors=balanced_y_scores,
    ...
    def log_metrics(self, metrics: Dict[str, float], epoch: int):
        ...
        formatted_metrics = [
            f"{key}: {str(round(value, 5))}" for key, value in self.metrics.items()
        ]
    ...
    def _is_best_checkpoint(self) -> bool:
        ...
        best_value = self.best_checkpoint["metrics"].get(
            self.training_config.target_metric,
            -np.inf if self.training_config.direction == "maximize" else np.inf,
        )
    ...
    def _check_early_stopping(self, epoch: int, is_best: bool) -> bool:
        ...
        if self.es_not_improved_epochs >= self.training_config.es_patience:
            logging.warning(
                f"Early stopping triggered. No improvement for {self.es_not_improved_epochs} epochs."
            )
            return True
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "early_stopping_triggered": (
                self.training_config.es_enabled
                and self.es_not_improved_epochs >= self.training_config.es_patience
            ),
