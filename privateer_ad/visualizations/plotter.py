...
class Visualizer:
    ...
    def visualize(
        self, y_true, y_pred, scores=None, threshold=None, target_names=None, prefix=""
    ):
        """
        ...
        self.figures[f"{prefix}_confusion_matrix"] = self.create_confusion_matrix(
            y_true, y_pred, target_names
        )
        self.figures[f"{prefix}_roc_curve"] = self.create_roc_curve(
            y_true, y_pred, scores, target_names
        )
        # Create distribution plot if we have scores and threshold
        if scores is not None and threshold is not None:
            self.figures[f"{prefix}_distribution"] = self.create_distribution_plot(
                scores, y_true, threshold
            )
    ...
    def create_distribution_plot(self, scores, y_true, threshold):
        ...
        df = pd.DataFrame(
            {"Score": scores, "Category": np.where(y_true == 0, "Benign", "Malicious")}
        )
        ...
        plt.title("Score Distribution", fontsize=14, pad=20)
