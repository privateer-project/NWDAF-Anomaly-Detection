                                            ...
                                            dbc.CardBody(
                                                [
                                                    ...
                                                    dbc.Checklist(
                                                        options=[
                                                            {
                                                                "label": "Show Malicious Devices",
                                                                "value": "malicious",
                                                            },
                                                            {
                                                                "label": "Show Benign Devices",
                                                                "value": "benign",
                                                            },
                                                        ],
                                                        value=[
                                            ...
                                            [
                                                ...
                                                html.P(
                                                    [
                                                        ...
                                                        html.Span(
                                                            "Green",
                                                            className="bg-success text-white px-2 rounded",
                                                        ),
                                                        " = Normal operation, ",
                                                        html.Span(
                                                            "Red",
                                                            className="bg-danger text-white px-2 rounded ml-2",
                                                        ),
                                                        " = Attack period",
                                                    ]
                                                ),
    ...
    def update_timeseries(device_types, metric):
        ...
        try:
            ...
            if len(df) > 0 and metric in df.columns:
                y_min = df[metric].min() - (
                    0.1 * abs(df[metric].min()) if df[metric].min() != 0 else 0.5
                )
                y_max = df[metric].max() + (
                    0.1 * abs(df[metric].max()) if df[metric].max() != 0 else 0.5
                )
            else:
                y_min = -1
                y_max = 1
            ...
            for device in unique_devices:
                ...
                if (
                    display_device
                    and len(device_df) > 0
                    and metric in device_df.columns
                ):
                    # Sort by time
            ...
            fig.update_layout(
                ...
                yaxis=dict(title=f"{metric_name}"),
                legend=dict(
