...
class PrivateerAnomalyDetector:
    ...
    def __init__(self, model_name: str = "TransformerAD_DP"):
        ...
        self.test_dl = self.data_processor.get_dataloader(
            "test", only_benign=False, train=True
        )
        ...
        self.model, self.threshold, self.loss_fn = load_champion_model(
            tracking_uri=self.mlflow_config.tracking_uri, model_name=self.model_name
        )
        ...
        try:
            ...
            with torch.no_grad():
                ...
                reconstruction_error = (
                    self.loss_fn(input_tensor, output).mean(dim=(1, 2)).item()
                )
            ...
            try:
                ...
                if len(input_flat.shape) == 2:  # [seq_len, features]
                    ...
                    for i, feature_name in enumerate(self.detector.input_features):
                        if i < len(current_features):
                            result["feature_values"][feature_name] = float(
                                current_features[i]
                            )
                ...
                try:
                    ...
                    device_id = self.detector.test_ds.transform_values(
                        "imeisv", sample[0]["groups"], inverse=True, group_id=True
                    )
                ...
                if device_id:
                    result["anonymized_device_id"] = (
                        f"anon-{hash(str(device_id)) % 10000}"
                    )
                else:
                    result["anonymized_device_id"] = (
                        f"anon-{(self.current_sample_index // 50) % 9}"
                    )
                # Put result in queue
...
min_threshold = float(np.floor(detector.threshold * 0.1))
max_threshold = float(np.ceil(detector.threshold * 10.0))
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H1(
                            "🛡️ PRIVATEER Network Anomaly Detection Demo",
                            className="text-center mb-4",
                        ),
                        html.P(
                            "Privacy-Preserving Anomaly Detection for 6G Networks",
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H4(
                                            "📊 Network Feature Values (Privacy-Preserved)",
                                            className="card-title",
                                        ),
                                        dcc.Graph(
                                            id="feature-display",
        ...
        [
            dbc.Col(
                [
                    ...
                    dbc.Badge(
                        f"Model: {detector.model_name}", color="info", className="fs-6"
                    ),
                    dbc.Badge(
                    ...
                    dbc.Badge(
                        f"Sample: {simulator.current_sample_index}",
                        color="secondary",
                        className="fs-6 ms-2",
                    ),
                ]
    ...
    if len(realtime_data["timestamp"]) > max_points:
        for key in realtime_data:
            if key == "feature_values":
                for feature in realtime_data["feature_values"]:
                    realtime_data["feature_values"][feature] = realtime_data[
                        "feature_values"
                    ][feature][-max_points:]
            else:
...
def create_feature_figure():
    ...
    feature_names = list(realtime_data["feature_values"].keys())[
        :6
    ]  # Show top 6 features

    for i, feature in enumerate(feature_names):
        if (
            feature in realtime_data["feature_values"]
            and realtime_data["feature_values"][feature]
        ):
            fig.add_trace(
                go.Scatter(
    ...
    if anomaly_times and feature_names:
        ...
        if first_feature in realtime_data["feature_values"]:
            anomaly_values = [
                realtime_data["feature_values"][first_feature][i]
                for i, anomaly in enumerate(realtime_data["is_anomaly"])
...
def create_anomaly_figure():
    ...
    true_anomaly_times = [
        realtime_data["timestamp"][i]
        for i, label in enumerate(realtime_data["true_label"])
        if label == 1
    ]
    true_anomaly_scores = [
        realtime_data["reconstruction_error"][i]
        for i, label in enumerate(realtime_data["true_label"])
        if label == 1
    ]
...
def create_statistics():
    ...
    true_positives = sum(
        1
        for i in range(len(realtime_data["is_anomaly"]))
        if realtime_data["is_anomaly"][i] and realtime_data["true_label"][i] == 1
    )

    true_positive_rate = (
        (true_positives / true_attacks) * 100 if true_attacks > 0 else 0
    )
    ...
    false_positive_rate = (
        (false_positives / normal_samples) * 100 if normal_samples > 0 else 0
    )
                        ...
                        [
                            dbc.CardBody(
                                [
                                    ...
                                    html.H3(
                                        f"{total_points}", className="text-primary"
                                    ),
                                ]
                            )
                        ]
                        ...
                        [
                            dbc.CardBody(
                                [
                                    html.H5("🚨 Detected"),
                                    html.H3(
                                        f"{detected_anomalies}", className="text-danger"
                                    ),
                                ]
                            )
                        ]
...
def create_device_list():
    ...
    # Create list items
    device_items = []
    for device_id, counts in sorted(device_counts.items())[
        -10:
    ]:  # Show last 10 devices
        anomaly_rate = (
            (counts["anomalies"] / counts["total"]) * 100 if counts["total"] > 0 else 0
        )
        color = (
            "danger"
            if anomaly_rate > 50
        ...
        device_items.append(
            dbc.ListGroupItem(
                [
                    ...
                    html.Small(
                        f"Samples: {counts['total']}, Anomalies: {counts['anomalies']}",
...
if __name__ == "__main__":
    ...
    logging.info("📄 Dataset: Loaded via DataProcessor.get_dataloader('test')")
